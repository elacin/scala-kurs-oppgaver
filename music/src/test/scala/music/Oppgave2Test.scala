...
class Oppgave2Test extends FunSuite {
  ...
  lazy val tingtings: Seq[Artist] =
    Music.parseSearchArtistResponse(loadFile("tingtings"))
  ...
  test("album amazonLinks") {
    artist.albums(0).amazonLink should be(
        "http://www.amazon.com/exec/obidos/redirect?link_code=ur2&tag=wikia-20&camp=1789&creative=9325&path=external-search%3Fsearch-type=ss%26index=music%26keyword=The%20Ting%20Tings%20We%20Started%20Nothing")
    artist.albums(1).amazonLink should be(
        "http://www.amazon.com/exec/obidos/redirect?link_code=ur2&tag=wikia-20&camp=1789&creative=9325&path=external-search%3Fsearch-type=ss%26index=music%26keyword=The_Ting_Tings%20Other%20Songs")
  }

  test("album songs") {
    artist.albums(0).songs should be(
        Seq("Great DJ",
            "That's Not My Name",
            "Fruit Machine",
            "Traffic Light",
    ...
    artist.albums(1).songs should be(
        Seq("Hands",
            "Hand",
            "Happy Birthday",
            "Hey",
